...
| [decisions/](decisions/) | Decisões adicionais | Decisões arquiteturais além dos documentos principais, incluindo execução, adaptação externa, composição operacional, referência de preço para custo pós-execução, configuração operacional da referência de preço e previsão de uso fornecida pelo operador para estimativa pré-execução. | Verifique antes de decisões arquiteturais; há decisões registradas de `0001` a `0007`. |
