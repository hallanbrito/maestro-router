...
| [decisions/](decisions/) | Decisões adicionais | Decisões arquiteturais além dos documentos principais, incluindo execução, adaptação externa, composição operacional, referência de preço, previsão de uso e configuração de múltiplas rotas OpenAI. | Verifique antes de decisões arquiteturais; há decisões registradas de `0001` a `0008`. |
