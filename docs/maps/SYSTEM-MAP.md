...
| ✅ Implementado | Primeiro adaptador externo | OpenAI Responses API por cliente assíncrono injetado (suporta múltiplas rotas via `MAESTRO_OPENAI_ROUTES_JSON`), sem registro no aplicativo padrão e sem rota, modelo ou credencial padrão. |
...
| ✅ Implementado | Estimativa operacional pré-execução | A composição OpenAI aceita uma previsão estática opcional (ou múltiplas rotas via `MAESTRO_OPENAI_ROUTES_JSON`) de `input_token` e `output_token`, fornecida e confirmada pelo operador. Com uma referência de preço válida, produz uma estimativa decimal exata e fixa no snapshot; sem previsão, ela permanece indisponível. A estimativa pode participar dos tetos existentes sem chamada externa e não garante o uso ou custo posterior. |
